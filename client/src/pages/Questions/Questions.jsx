...
import { HiPlus, HiCheckCircle, HiThumbUp, HiChat } from 'react-icons/hi';
...
const Questions = () => {
  ...
  useEffect(() => { API.get('/questions').then(({ data }) => setQuestions(data.questions)).catch(() => {}).finally(() => setLoading(false)); }, []);
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      setShowCreate(false); setForm({ title: '', content: '', category: 'other', tags: '' });
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      <div className="flex items-center justify-between" style={{ marginBottom: 'var(--space-lg)' }}>
        <h1 className="heading-2">❓ Q&A Hub</h1>
        <button className="btn btn-primary" onClick={() => setShowCreate(!showCreate)}><HiPlus /> Ask Question</button>
      </div>
          ...
          <form onSubmit={handleCreate} className="flex flex-col gap-md">
            ...
            <div className="flex gap-sm">
              <select className="form-input" value={form.category} onChange={e => setForm({ ...form, category: e.target.value })}>
                {['tech', 'science', 'arts', 'health', 'business', 'education', 'lifestyle', 'other'].map(c => <option key={c} value={c}>{c}</option>)}
              </select>
      ...
      {loading ? <div className="loader"><div className="spinner spinner-lg"></div></div> : questions.length === 0 ? (
        <div className="empty-state"><div className="empty-state-icon">❓</div><div className="empty-state-title">No questions yet</div><div className="empty-state-text">Be the first to ask!</div></div>
      ) : (
            ...
            <div key={q._id} className="card" style={{ cursor: 'pointer' }} onClick={() => setExpandedQ(expandedQ === q._id ? null : q._id)}>
              <div className="flex items-start gap-md">
                ...
                <div className="flex-1">
                  ...
                  <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)' }}>
                    Asked by {q.author?.firstName} {q.author?.lastName} · {formatDistanceToNow(new Date(q.createdAt), { addSuffix: true })} · {q.views} views
                    {q.rewardPoints > 0 && <span className="badge badge-warm" style={{ marginLeft: 8 }}>🏆 {q.rewardPoints} pts</span>}
                  </div>
                  {q.tags?.length > 0 && <div className="flex gap-xs" style={{ marginTop: 8 }}>{q.tags.map(t => <span key={t} className="badge badge-secondary">{t}</span>)}</div>}
                </div>
                <span className={`badge ${q.status === 'closed' ? 'badge-success' : q.status === 'answered' ? 'badge-primary' : 'badge-warm'}`}>{q.status}</span>
              </div>
              ...
              {expandedQ === q._id && (
                <div style={{ marginTop: 'var(--space-md)', borderTop: '1px solid var(--border)', paddingTop: 'var(--space-md)' }} onClick={e => e.stopPropagation()}>
                  ...
                  {q.answers?.map(a => (
                    <div key={a._id} style={{ padding: 'var(--space-sm) var(--space-md)', background: a._id === q.bestAnswer ? 'rgba(34,197,94,0.08)' : 'var(--bg-tertiary)', borderRadius: 'var(--radius-md)', marginBottom: 'var(--space-xs)', borderLeft: a._id === q.bestAnswer ? '3px solid var(--success)' : 'none' }}>
                      <div className="flex items-center gap-xs" style={{ marginBottom: 4 }}><strong style={{ fontSize: 'var(--text-sm)' }}>{a.author?.firstName} {a.author?.lastName}</strong>{a._id === q.bestAnswer && <span className="badge badge-success">✅ Best</span>}</div>
