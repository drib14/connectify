  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      ...
      <section style={{ marginBottom: 'var(--space-2xl)' }}>
        <h2 className="heading-4" style={{ marginBottom: 'var(--space-md)', display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 'var(--space-md)' }}>
          {suggestedUsers.map(u => (
            <Link key={u._id} to={`/profile/${u.username}`} className="card card-interactive" style={{ textAlign: 'center', textDecoration: 'none', color: 'inherit' }}>
              {u.avatar ? (
                <img src={u.avatar} className="avatar avatar-xl" alt="" style={{ margin: '0 auto var(--space-sm)' }} />
              ) : (
                <div className="avatar avatar-xl avatar-placeholder" style={{ margin: '0 auto var(--space-sm)', fontSize: '1.5rem' }}>
                  {u.firstName?.[0]}
                  {u.lastName?.[0]}
                </div>
              )}
              ...
              {u.contributionScore > 0 && (
                <div className="badge badge-primary" style={{ marginTop: 8, display: 'inline-flex', alignItems: 'center', gap: '3px' }}>
                  <FaStar style={{ fontSize: '10px' }} /> {u.contributionScore} pts
                </div>
              )}
              {u.skills?.length > 0 && (
                <div className="flex gap-xs justify-center flex-wrap" style={{ marginTop: 8 }}>
      ...
      <section style={{ marginBottom: 'var(--space-2xl)' }}>
        <div className="flex items-center justify-between" style={{ marginBottom: 'var(--space-md)' }}>
          <h2 className="heading-4" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
            <HiFlag /> Public Goals
          </h2>
          <Link to="/goals" className="auth-link" style={{ fontSize: 'var(--text-sm)' }}>View all →</Link>
        </div>
        {publicGoals.length === 0 ? (
          <p style={{ color: 'var(--text-tertiary)', fontSize: 'var(--text-sm)' }}>No public goals yet.</p>
        ) : (
              ...
              <div key={g._id} className="card flex items-center gap-md" style={{ padding: 'var(--space-md)' }}>
                <div className="flex-1">
                  ...
                  <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-tertiary)' }}>
                    by {g.user?.firstName} {g.user?.lastName} · {g.category.charAt(0).toUpperCase() + g.category.slice(1)}
                  </div>
                </div>
                <div style={{ minWidth: 100 }}>
                  ...
                  <div style={{ fontSize: 'var(--text-xs)', textAlign: 'right', marginTop: 4, color: 'var(--primary)' }}>
                    {g.progress}%
                  </div>
                </div>
              </div>
      ...
      <section style={{ marginBottom: 'var(--space-2xl)' }}>
        <div className="flex items-center justify-between" style={{ marginBottom: 'var(--space-md)' }}>
          <h2 className="heading-4" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
            <HiUserGroup /> Popular Communities
          </h2>
